...
const rotateVectorAboutAxis = (vector, axis, angle) => {
  let rotationMatrix;
  switch (axis) {
    case 'x':
...
class FilamentViewer extends LitElement {
  ...
  updated(props) {
    ...
    if (props.has('materialVariant') && this.asset)
  ...
  _loadAsset() {
    ...
    if (this.srcBlob && this.srcBlob.name.endsWith('.glb')) {
      this.srcBlob.arrayBuffer().then((buffer) => {
        ...
        const x = (aabb.min[0] + aabb.max[0]) / 2;
        // const y = (aabb.min[1] + aabb.max[1]) / 2;
        const y = -0.125;
        const z = (aabb.min[2] + aabb.max[2]) / 2;
        const eye = [0, 0, 1.5];
        const center = [0, 0, 0];
        const up = [0, 1, 0];
        this.camera.lookAt(eye, center, up);
    ...
    if (this.srcBlob && this.srcBlob.name.endsWith('.gltf')) {
      ...
      this.srcBlob.arrayBuffer().then((buffer) => {
        ...
        const resourceLoader = new Filament.gltfio$ResourceLoader(
          this.engine,
  ...
  _updateAsset() {
    ...
    if (this.trackball) {
      rootTransform = Filament.multiplyMatrices(
        rootTransform,
        this.trackball.getMatrix()
      );
    }
